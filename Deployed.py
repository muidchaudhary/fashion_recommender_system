...
def recommend(features, feature_list):
    dimensions = len(features)  # Assuming 'features' is a numpy array or list
    annoy_index = AnnoyIndex(dimensions, metric='euclidean')  # Build an Annoy index with the same dimensions as the input features

    for i, feature in enumerate(feature_list):  # Add items to the Annoy index
        annoy_index.add_item(i, feature)

    annoy_index.build(n_trees=10)  # Build the index (moved outside the loop for correct indentation)
